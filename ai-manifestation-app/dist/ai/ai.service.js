...
let AIService = class AIService {
    ...
    async optimizeWish(title, description, focus) {
        const prompt = `
    ...
    async generateImage(text, keywords) {
        const imageSeeds = [
    ...
    async generateSMARTGoals(wishText) {
        const prompt = `
    ...
    async generateInspirationQuote(context) {
        const quotes = [
    ...
    async analyzeGratitudeSentiment(content) {
        const positiveWords = ['感恩', '感谢', '快乐', '幸福', '爱', '温暖', '美好', '满足', '珍惜', '幸运'];
        const words = content.toLowerCase().split(/\s+/);
        const foundKeywords = words.filter(word => positiveWords.some(pw => word.includes(pw)));
